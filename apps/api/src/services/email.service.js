...
async function sendWelcome(to, schoolName, loginUrl) {
  await transporter.sendMail({
    from: `"EduBridge" <${process.env.SMTP_FROM}>`,
    to,
    subject: `Welcome to EduBridge — ${schoolName}`,
    html: `
      ...
      <p>Your application is pending EduBridge approval. You will be notified once approved.</p>
    `,
  });
}
...
async function sendApprovalNotification(to, schoolName, loginUrl) {
  await transporter.sendMail({
    ...
    html: `
      ...
      <p><strong>${schoolName}</strong> has been approved on EduBridge.</p>
      <p><a href="${loginUrl}">Login to your dashboard</a></p>
    `,
  });
}
...
async function sendInvite(to, firstName, schoolName, role, inviteUrl) {
  ...
  await transporter.sendMail({
    from: `"EduBridge" <${process.env.SMTP_FROM}>`,
    to,
    subject: `You've been invited to join ${schoolName} on EduBridge`,
    html: `
      <h2>Hello, ${firstName}!</h2>
      <p>You have been invited to join <strong>${schoolName}</strong> as a ${roleLabel} on EduBridge.</p>

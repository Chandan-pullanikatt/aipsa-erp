...
export default function StudentLmsPage() {
  ...
  useEffect(() => {
    async function loadSubjects() {
      try {
        ...
        console.error(err);
        setError(err.response?.data?.error || 'Failed to fetch course schedules.');
      } finally {
  ...
  // Fetch materials timeline for selected subject
    ...
    <div className="max-w-6xl space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-gray-100 pb-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500 font-body">Browse course material timelines, search subjects, and download materials.</p>
        </div>
      ...
      {subjects.length === 0 ? (
        <div className="py-24 text-center border border-[#E5E7EB] bg-white rounded-xl p-6 shadow-sm font-body">
          ...
          <p className="text-xs text-gray-400 mt-2 max-w-sm mx-auto leading-relaxed">
            Your class currently has no courses mapped in the learning management system. Ask your School Administrator or Teacher to publish curriculum chapters.
          </p>
        </div>
      ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1 flex-1">
                          ...
                          <p className={`text-[10px] mt-0.5 ${isSelected ? 'text-[#1D7A4A]/80' : 'text-gray-500'}`}>
                            Class {sub.class?.name || 'Class Assigned'}
                          </p>
                        </div>
                      ...
                      <div className={`mt-4 pt-3 border-t text-[10px] font-bold flex items-center justify-between ${
                        ...
                        <span>
                          Instructor: {sub.teacher ? `${sub.teacher.firstName} ${sub.teacher.lastName}` : 'Guest Lecturer'}
                        </span>
            ...
            <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-gray-400 font-body">Syllabus timeline & materials sequence</p>
                </div>
              ...
              {loadingMaterials ? (
                ...
                <div className="py-16 text-center border border-dashed border-[#E5E7EB] rounded-xl font-body">
                  ...
                  <p className="text-sm font-semibold text-gray-750 font-display">Syllabus Outlines Coming Soon</p>
                  <p className="text-xs text-gray-400 max-w-xs mx-auto mt-1 leading-relaxed">
                    No course material outlines or study assets have been published by your mentor yet.
                  </p>
                </div>
              ) : (
                <div className="space-y-6 relative before:absolute before:inset-y-1 before:left-3.5 before:w-0.5 before:bg-gray-100">
                  {materials.map((mat) => (
                    <div key={mat.id} className="relative pl-10 flex flex-col items-start justify-between gap-4 font-body">
                      <div className="absolute left-2.5 top-1.5 w-2.5 h-2.5 rounded-full bg-[#1D7A4A] ring-4 ring-[#E5F6EE] shrink-0 transform -translate-x-1/2"></div>
                      ...
                      <div className="space-y-2 flex-1 w-full">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <h4 className="font-bold text-slate-800 text-base leading-snug font-display">{mat.title}</h4>
                        </div>
                        ...
                        {(mat.attachmentUrl || (mat.educationalLinks && mat.educationalLinks.length > 0)) && (
                          <div className="bg-[#F9FAFB] border border-[#E5E7EB] rounded-xl p-4 mt-3 flex flex-wrap gap-4 text-xs w-full">
                            {mat.attachmentUrl && (
                              <div className="flex flex-col gap-1.5 w-full sm:w-auto">
                                <span className="font-bold text-[10px] text-gray-400 uppercase tracking-wider font-display">Attachments</span>
                                <a
                            ...
                            {mat.educationalLinks && mat.educationalLinks.length > 0 && (
                              <div className="flex flex-col gap-1.5 w-full sm:w-auto">
                                <span className="font-bold text-[10px] text-gray-400 uppercase tracking-wider block font-display">Educational reference assets</span>

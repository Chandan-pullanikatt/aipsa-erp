...
function ParentDashboardContent() {
  ...
  // Homework submission modal (parent submits on behalf of the active child)
  ...
  // Submit homework on behalf of the active child. The backend resolves the
  // student from studentId, validated against this parent's guardian link.
  const handleSubmitHomework = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!submitHwItem || !activeStudent) return;
    setSubmitting(true);
    try {
  ...
  return (
    <div className="max-w-6xl space-y-6 font-body">
                                  ...
                                  <div className="flex items-center gap-3">
                                    {submittedIds.has(hw.id) ? (
                                      ...
                                      <button
                                        ...
                                        className="inline-flex items-center gap-1 text-[10px] font-bold text-white bg-[#1D7A4A] hover:bg-[#155B37] px-2.5 py-1 rounded-full transition-colors cursor-pointer font-display"
                                      >
      ...
      {/* POPUP: HOMEWORK SUBMISSION MODAL (parent submits for the active child) */}
      {submitHwItem && (
            ...
            <div className="bg-[#1A1D23] px-6 py-4 flex items-center justify-between text-white">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5 truncate max-w-xs">
                  {submitHwItem.title}
                  {activeStudent && <span> · {activeStudent.firstName}</span>}
                </p>
              </div>
              <button

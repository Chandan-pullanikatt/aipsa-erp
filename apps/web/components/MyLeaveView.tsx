...
interface Props {
  title: string;
  subtitle: string;
  /** Message shown while the first fetch is in flight. */
  loadingLabel?: string;
}

/**
 * Self-service leave screen shared by every staff-side role (STAFF, TEACHER).
 * Requests land in the admin's HR › Leave Approvals and Attendance › Leave
 * Requests tabs, which drive PATCH /attendance/leave/:id/review.
 */
export default function MyLeaveView({ title, subtitle, loadingLabel = 'Loading your leave requests...' }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess('Leave request sent to the school admin for approval.');
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-[#E5E7EB]">
        <div>
          <h1 className="font-display text-[32px] font-bold leading-tight text-[#1A1D23]">{title}</h1>
          <p className="font-body text-[14px] text-[#6B7280] mt-1">{subtitle}</p>
        </div>
        <button
